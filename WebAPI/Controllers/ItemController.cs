...
namespace WebAPI.Controllers
{
    ...
    public class ItemController : ApiController
    {
        ...
        [ResponseType(typeof(RespuestaVMR<List<ItemVMR>>))]
        [Route("api/item/no-paginados")]
        public IHttpActionResult GetsNoPag()
        {
            var respuesta = new RespuestaVMR<List<ItemVMR>>();

            try
            {
                respuesta.datos = ItemBLL.ObtenerProductosActivos();
            }
            catch (Exception e)
            {
        ...
        /// <response code="404">Recurso no encontrado</response>
        /// <response code="500">Error interno del servidor</response>
        [ResponseType(typeof(RespuestaVMR<ListadoPaginadoVMR<ItemVMR>>))]
        public IHttpActionResult Gets(int cantidad = 20, int pagina = 0, string textoBusqueda = null)
        {
            var respuesta = new RespuestaVMR<ListadoPaginadoVMR<ItemVMR>>();

            try
            {
                respuesta.datos = ItemBLL.Gets(cantidad, pagina, textoBusqueda);
            }
            catch (Exception e)
            {
                respuesta.codigo = HttpStatusCode.InternalServerError;
        ...
        /// <response code="404">Recurso no encontrado</response>
        /// <response code="500">Error interno del servidor</response>
        [ResponseType(typeof(RespuestaVMR<ItemVMR>))]
        public IHttpActionResult Get(long id)
        {
            var respuesta = new RespuestaVMR<ItemVMR>();

            try
            {
                respuesta.datos = ItemBLL.Get(id);
            }
            catch (Exception e)
            {
                respuesta.codigo = HttpStatusCode.InternalServerError;
        ...
        public IHttpActionResult Post(Item item)
        {
            ...
            try
            {
                respuesta.datos = ItemBLL.Post(item);
            }
            catch (Exception e)
            {
                respuesta.codigo = HttpStatusCode.InternalServerError;
        ...
        public IHttpActionResult Put(long id, ItemVMR item)
        {
